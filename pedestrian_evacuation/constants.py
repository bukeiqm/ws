...
# 不同年龄段的行人具有不同的心理承受能力
STIMULUS_THRESHOLD = {
    "adult": 10.0,   # 成人心理阈值（用于调试）
    "elder": 10.0,   # 老人心理阈值（用于调试，老人更容易恐慌）
    "child": 10.0    # 儿童心理阈值（用于调试）
}
# 向后兼容：保留全局阈值作为默认值
STIMULUS_THRESHOLD_DEFAULT = 0.8  # 默认刺激阈值（向后兼容）

# 目标点选择参数（用于调试）
SMART_TARGET_SELECTION = False  # 是否使用智能逻辑选择目标点（与路径选择逻辑相同）
# 如果为True，目标点选择会使用与路径选择相同的智能评分系统
# 如果为False，目标点直接设置为路径中下一个节点的中心点（默认行为）

# 监视器参数（用于调试）
MONITOR_RESET_FRAMES = 0  # 监视器重新绘制的帧数间隔（0表示不重新绘制）
# 如果设置为大于0的值，监视器会在绘制指定帧数后清除历史数据并重新开始绘制
# 例如：设置为300表示每300帧（30秒，假设每帧0.1秒）重新绘制一次
